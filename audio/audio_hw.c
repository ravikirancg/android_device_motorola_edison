...
#define PRODUCT_DEVICE_UMTS_SPYDER "umts_spyder"
...
static int get_boardtype(struct omap4_audio_device *adev)
{
    ...
    else if(!strcmp(board, PRODUCT_DEVICE_UMTS_SPYDER)){
           adev->board_type = UMTS_SPYDER;
           adev->sidetone_capture = 0;      
    }
    else
        return -EINVAL;
                     ...
                     hw_device_t** device)
{
    ...
    if (!adev->mixer_ctls.mm_dl1)
      ALOGE("Unable to locate all mixer controls : mixer_ctls.mm_dl1."); 
    if(!adev->mixer_ctls.vx_dl1)
      ALOGE("Unable to locate all mixer controls : mixer_ctls.vx_dl1."); 
    if(!adev->mixer_ctls.mm_dl2)
      ALOGE("Unable to locate all mixer controls : mixer_ctls.mm_dl2."); 
    if(!adev->mixer_ctls.vx_dl2)
      ALOGE("Unable to locate all mixer controls : mixer_ctls.vx_dl2."); 
    if(!adev->mixer_ctls.dl2_mono)
      ALOGE("Unable to locate all mixer controls : mixer_ctls.dl2_mono."); 
    if(!adev->mixer_ctls.dl1_mono)
      ALOGE("Unable to locate all mixer controls : dl1_mono."); 
    if(!adev->mixer_ctls.dl1_headset)
      ALOGE("Unable to locate all mixer controls : dl1_headset."); 
    if(!adev->mixer_ctls.dl1_bt )
      ALOGE("Unable to locate all mixer controls : mixer_ctls.dl1_bt."); 
    if(!adev->mixer_ctls.earpiece_enable)
      ALOGE("Unable to locate all mixer controls : earpiece_enable."); 
    if(!adev->mixer_ctls.left_capture)
      ALOGE("Unable to locate all mixer controls : left_capture."); 
    if( !adev->mixer_ctls.right_capture)
      ALOGE("Unable to locate all mixer controls : right_capture."); 
    if(!adev->mixer_ctls.amic_ul_volume)
      ALOGE("Unable to locate all mixer controls : amic_ul_volume."); 
    if(!adev->mixer_ctls.sidetone_capture)
      ALOGE("Unable to locate all mixer controls : sidetone_capture."); 
    if(!adev->mixer_ctls.headset_volume)
      ALOGE("Unable to locate all mixer controls : headset_volume."); 
    if(!adev->mixer_ctls.speaker_volume)
      ALOGE("Unable to locate all mixer controls : speaker_volume."); 
    if(!adev->mixer_ctls.dmic1_ul_volume)
      ALOGE("Unable to locate all mixer controls : .dmic1_ul_volume."); 
    if(!adev->mixer_ctls.dl1_eq)
      ALOGE("Unable to locate all mixer controls :.dl1_eq."); 
    if(!adev->mixer_ctls.voice_ul_volume)
      ALOGE("Unable to locate all mixer controls : voice_ul_volume."); 
